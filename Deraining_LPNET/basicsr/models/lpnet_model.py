...
from basicsr.models.archs.lpnet_arch import GaussianPyramid

loss_module = importlib.import_module('basicsr.models.losses')
metric_module = importlib.import_module('basicsr.metrics')


class LPNetModel(BaseModel):
    """Base Deblur model for single image deblur."""

    def __init__(self, opt):
        super(LPNetModel, self).__init__(opt)

        # define network
        self.net_g = define_network(deepcopy(opt['network_g']))
        self.net_g = self.model_to_device(self.net_g)
        # self.print_network(self.net_g)
        ssim_loss_cls = getattr(loss_module, 'SSIMLoss')
        self.ssim_loss = ssim_loss_cls().to(self.device)
        ...
        self.kernel = torch.tensor(self.kernel).to(self.device)
        self.num_pyramids = 5
    ...
    def optimize_parameters(self, current_iter):
        ...
        gt_pyramid =GaussianPyramid(self.gt, self.kernel, (self.num_pyramids - 1))
        ...
        # print(gt_pyramid[0].size())
        # print(gt_pyramid[1].size())
        ...
        l0 = self.cri_pix(preds[0], gt_pyramid[0])
        l1 = self.cri_pix(preds[1], gt_pyramid[1])
        l2 = self.cri_pix(preds[2], gt_pyramid[2])
        l3 = self.cri_pix(preds[3], gt_pyramid[3])
        l4 = self.cri_pix(preds[-1], self.gt)

        l3_ssim_loss = self.ssim_loss(preds[3], gt_pyramid[3])
        l4_ssim_loss = self.ssim_loss(preds[-1], self.gt)
        loss_dict['l0'] = l0
        loss_dict['l1'] = l1
        loss_dict['l2'] = l2
        loss_dict['l3'] = l3
        ...
        loss_dict['l3_ssim'] = l3_ssim_loss
        loss_dict['l4_ssim'] = l4_ssim_loss
        l_total = l0 + l1 + l2 + l3 + l4 + 0.5*l3_ssim_loss + 0.5*l4_ssim_loss
        # l_total =  l4 + 0.5*l4_ssim_loss
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger,
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            # print('self.opt[val].get(grids)',self.opt['val'].get('grids'), type(self.opt['val'].get('grids')))
