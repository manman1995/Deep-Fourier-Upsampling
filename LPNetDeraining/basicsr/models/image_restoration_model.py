...
class ImageRestorationModel(BaseModel):
    ...
    def setup_optimizers(self):
        ...
        optim_params = []
        optim_params_lowlr = []
        optim_params_low_band = []
        for k, v in self.net_g.named_parameters():
            if v.requires_grad:
                if k.startswith('module.offsets') or k.startswith('module.dcns'):
                    optim_params_lowlr.append(v)
                # elif k.startswith('low_band_net'):
                #     optim_params_low_band.append(v)
                else:
    ...
    def optimize_parameters(self, current_iter):
        ...
        l1 = self.cri_pix(preds[0], self.gt)
        l2 = self.cri_pix(preds[1], self.gt)
        loss_dict['l1'] = l1 + l2
        loss_content = l1 + l2

        # pred_fft1 = torch.rfft(preds[0], signal_ndim=2, normalized=False, onesided=False)
        # pred_fft2 = torch.rfft(preds[1], signal_ndim=2, normalized=False, onesided=False)
        # gt_fft = torch.rfft(self.gt, signal_ndim=2, normalized=False, onesided=False)
        #
        #
        # f1 = self.cri_pix(pred_fft1, gt_fft)
        # f2 = self.cri_pix(pred_fft2, gt_fft)
        # loss_dict['fft1'] = f1
        # loss_dict['fft2'] = f2
        # loss_fft = f1 + f2
        # l_total = loss_content + 0.1 * loss_fft
        l_total = loss_content
        # perceptual loss
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            while i < n:
                ...
                # Yl, Yh = self.xfm(self.gt)
                # gt_low_band = Yl / 2
                # gt_hv_band = (Yh[0][:, :, 0, :, :] + 1) / 2
                # gt_hh_band = (Yh[0][:, :, 1, :, :] + 1) / 2
                # gt_hd_band = (Yh[0][:, :, 2, :, :] + 1) / 2
                pred = self.net_g(self.lq[i:j, :, :, :])
                # low_band_out_1, low_band_out_2, hv_band_out_1, hv_band_out_2, hh_band_out_1, hh_band_out_2, hd_band_out_1, hd_band_out_2, derain_out_1, derain_out_2 = pred
                # hv_band_tmp_2 = hv_band_out_2[:, :, None, :, :] * 2 - 1
                # hh_band_tmp_2 = hh_band_out_2[:, :, None, :, :] * 2 - 1
                # hd_band_tmp_2 = hd_band_out_2[:, :, None, :, :] * 2 - 1
                # h_band_tmp_2 = torch.cat((hv_band_tmp_2, hh_band_tmp_2, hd_band_tmp_2), dim=2)

                if isinstance(pred, list):
                    pred = pred[-1]
                    # pred = self.ifm((low_band_out_2*2,[h_band_tmp_2]))
                # pred_output =  tensor2img(low_band_out_2, rgb2bgr=True)
                # gt_output = tensor2img(gt_low_band, rgb2bgr=True)
                # self.tmp_sum += getattr(metric_module, 'calculate_psnr')(pred_output, gt_output, 0, test_y_channel=True)
                # self.tmp_count += 1
                # print(self.tmp_sum/self.tmp_count)
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger,
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            # print('H,W', h, w)
            val_data['lq'] = val_data['lq'][:,:, :(h - h % 16), :(w - w % 16)]
            val_data['gt'] = val_data['gt'][:, :, :(h - h % 16), :(w - w % 16)]

            # print('val_data .. ', val_data['lq'].size(), val_data['gt'].size())
            self.feed_data(val_data)
            if self.opt['val'].get('grids'):
                self.grids()
