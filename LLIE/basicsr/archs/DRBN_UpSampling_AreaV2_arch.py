...
class frescat_area(nn.Module):
    def __init__(self, channels_in, channels_out):
        ...
        self.up_f = freup_Areadinterpolation(channels_in, channels_out)
    ...
    def forward(self,x):
        ...
        x1 = self.up_f(x)
        x2 = self.up(x)
        xn = self.fuse(torch.cat([x1,x2],dim=1))
...
class DRBN_UpSampling_AreaV2(nn.Module):
    def __init__(self, n_color):
        super(DRBN_UpSampling_AreaV2, self).__init__()
