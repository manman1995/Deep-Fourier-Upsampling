...
class Net(nn.Module):
    def __init__(self, num_channels, base_filter, args,mode="C"):
        ...
        self.relu = nn.ReLU(inplace=True)
        self.freop1 = freup_Cornerdinterpolation(64)
        self.freop2=freup_Cornerdinterpolation(128)
        self.fsfuse1 = nn.Conv2d(64*2,64,1,1,0)
        self.fsfuse2 = nn.Conv2d(128*2, 128, 1, 1, 0)
        ################################################################################################
        # stage 1
    ...
    def forward(self,lms,_,pan):
        ...
        x1_fre = self.freop1(x[1])
        x2_fre = self.freop2(self.freop2(x[2]))
        x1 = self.fsfuse1(torch.cat([x1, x1_fre], dim=1))
        x2 = self.fsfuse2(torch.cat([x2, x2_fre], dim=1))
