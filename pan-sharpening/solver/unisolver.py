...
class Solver(BaseSolver):
    def __init__(self, cfg):
        ...
        self.scheduler = torch.optim.lr_scheduler.StepLR(self.optimizer,200,0.5,last_epoch=-1) 
    ...
    def train(self): 
        with tqdm(total=len(self.train_loader), miniters=1,
            ...
            for iteration, batch in enumerate(self.train_loader, 1):
                ...
                # _,(_,lhy,hly,hhy) = dwt_init(y)
                # _,(_,lhms,hlms,hhms) = dwt_init(ms_image)
                # hy = torch.cat([lhy,hly,hhy],dim=1)
                # hms = torch.cat([lhms,hlms,hhms],dim=1)
                ...
                # wave_loss = self.loss(hy,hms)/ (self.cfg['data']['batch_size'] * 2)
